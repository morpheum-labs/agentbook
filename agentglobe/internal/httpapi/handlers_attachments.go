...
func (s *Server) listPostAttachments(db *gorm.DB, postID string) []map[string]any {
	var rows []dbpkg.Attachment
	_ = db.Where("post_id = ? AND comment_id IS NULL", postID).Order("created_at ASC").Find(&rows).Error
...
func (s *Server) attachmentsByCommentIDs(db *gorm.DB, ids []string) map[string][]map[string]any {
	if len(ids) == 0 {
		return map[string][]map[string]any{}
	}
	var rows []dbpkg.Attachment
	_ = db.Where("comment_id IN ?", ids).Order("created_at ASC").Find(&rows).Error
...
func (s *Server) handleListPostAttachments(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.dbCtx(r).First(&dbpkg.Post{}, "id = ?", postID).Error; err != nil {
		writeDetail(w, http.StatusNotFound, "Post not found")
		return
	}
	writeJSON(w, http.StatusOK, s.listPostAttachments(s.dbCtx(r), postID))
}

func (s *Server) handleListCommentAttachments(w http.ResponseWriter, r *http.Request) {
	commentID := chi.URLParam(r, "commentID")
	var c dbpkg.Comment
	if err := s.dbCtx(r).First(&c, "id = ?", commentID).Error; err != nil {
		writeDetail(w, http.StatusNotFound, "Comment not found")
		return
	}
	var rows []dbpkg.Attachment
	_ = s.dbCtx(r).Where("comment_id = ?", commentID).Order("created_at ASC").Find(&rows).Error
	out := make([]map[string]any, 0, len(rows))
	for i := range rows {
		out = append(out, s.attachmentMeta(&rows[i]))
	}
...
func (s *Server) handleUploadPostAttachment(w http.ResponseWriter, r *http.Request) {
	...
	postID := chi.URLParam(r, "postID")
	var post dbpkg.Post
	if err := s.dbCtx(r).First(&post, "id = ?", postID).Error; err != nil {
		writeDetail(w, http.StatusNotFound, "Post not found")
		return
	}
...
func (s *Server) handleUploadCommentAttachment(w http.ResponseWriter, r *http.Request) {
	...
	var post dbpkg.Post
	if err := s.dbCtx(r).First(&post, "id = ?", c.PostID).Error; err != nil {
		writeDetail(w, http.StatusNotFound, "Post not found")
		return
	}
	pid := c.PostID
