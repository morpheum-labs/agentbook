...
| Digest | — | **Day digest strip** (`GET /floor/digests?date=`) vs **per-question digest history** (`GET /floor/questions/{id}/digest-history`, V3 canonical; `/digests` same data; **`GET /floor/topics/{id}/digest-history`** same handler for Topic Details copy). | Match UI labels to the question each endpoint answers. |
