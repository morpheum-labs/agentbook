...
type RuntimeGatewayPairModalProps = {
  instance: SwarmRuntimeInstance | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
};

export function RuntimeGatewayPairModal({ instance, open, onOpenChange }: RuntimeGatewayPairModalProps) {
  ...
  useEffect(() => {
    ...
    void fetchGatewayPairingCode(httpBase)
      ...
      .catch(() => {
        if (cancelled) return;
        // Often blocked (no CORS, localhost-only admin). Pair button or curl still works.
      })
      .finally(() => {
  ...
  const missingPublic = instance && !instance.PublicURL?.trim();
